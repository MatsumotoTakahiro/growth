...
if len(argv) < 2:
    print('Usage: python fit_cr.py <dataset_name> <cr_binwidth (count/sec)> <Const> <Mean> <Sigma>')
    quit()
...
if __name__ == '__main__':
   cr_hist.Fit("gaus")
   func = ROOT.gROOT.FindObject("gaus")
   func.SetParameters(constt,meann,sigmaa)
   cr_hist.Fit("gaus","","",meann-3*sigmaa,meann+2*sigmaa)
   func.SetParameters(func.GetParameter(0),func.GetParameter(1),func.GetParameter(2))
   cr_hist.Fit("gaus","","",func.GetParameter(1)-3*func.GetParameter(2),func.GetParameter(1)+2*func.GetParameter(2))
   rep = ''
