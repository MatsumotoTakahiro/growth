...
#error
if len(argv) < 2:
    print('Usage: python pi_making.py <fitsfile> <adcchannel> <start_time> <end_time>(%Y/%m/%d %H:%M:%S) <event_name>')
    quit()
    
#input
filename = argv[1]
adcchannel = int(argv[2])
start_time = datetime.strptime(argv[3], '%Y/%m/%d %H:%M:%S').timestamp()
end_time = datetime.strptime(argv[4], '%Y/%m/%d %H:%M:%S').timestamp()
event_name = argv[5]
...
#pi
bin_number = 2048
hist = np.histogram(data.energy, bins=bin_number, range=(40, 41000))
...
spectrum.header.append(('EVENTDAT',datetime.strptime(argv[3], '%Y/%m/%d %H:%M:%S').strftime('%Y%m%d'),'event date (JST)'))
