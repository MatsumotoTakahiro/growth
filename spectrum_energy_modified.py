...
#input
if len(sys.argv) < 2:
  print("Usage: python spectrum_energy_modified.py <input file> <adcchannel> <start_time> <end_time>(%Y/%m/%d %H:%M:%S:%f(5-digit))")
  quit()

energy_min = 0.4
energy_max = 24.0
energy_bin_number = 160
...
dead_event = trigger_data.delta_trigger.sum() - len(trigger_data) - trigger_data.delta_trigger[len(trigger_data.index)-1]
...
scale_factor = (event_number + dead_event) / event_number
